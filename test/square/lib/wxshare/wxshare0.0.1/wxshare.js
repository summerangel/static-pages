...
var wxapi=function(){
	if(!RegExp("MicroMessenger").test(navigator.userAgent)) return;
	this.shareinfo={
		title: '', // 分享标题
		desc: '', // 分享描述
		link: '', // 分享链接
	...
	// $.getScript("http://res.wx.qq.com/open/js/jweixin-1.0.0.js",this.init())
	this.init()
}
...
wxapi.prototype.wxshare=function(arr,fun){
	for (id in arr) {
		this.shareinfo[id] = arr[id];
	}
	var wxs = this.shareinfo;
	wx.onMenuShareTimeline({
		title: wxs.title, // 分享标题
		link: wxs.link, // 分享链接
		imgUrl: wxs.imgUrl, // 分享图标
		success: function() {
			if(typeof fun.timeline_s =="function" ){
				fun.timeline_s()
			}
		},
		cancel: function() {
			if(typeof fun.timeline_c =="function" ){
				fun.timeline_c()
			}
		}
	});
	wx.onMenuShareAppMessage({
		...
		dataUrl: '', // 如果type是music或video，则要提供数据链接，默认为空
		success: function() {
			if(typeof fun.message_s =="function" ){
				fun.message_s()
			}
		},
		cancel: function() {
			if(typeof fun.message_c =="function" ){
				fun.message_c()
			}
		}
	});
